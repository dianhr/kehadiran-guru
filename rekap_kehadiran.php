...
// Hitung total baris detail
$sql_total = "SELECT COUNT(*) as total FROM kehadiran_guru13";
if ($where) {
...
$sql_detail .= " ORDER BY tanggal DESC LIMIT $limit OFFSET $offset";
...
<nav aria-label="Page navigation">
  <ul class="pagination justify-content-center">
    ...
    <?php
    $params = $_GET; // Bawa filter GET
    $prev = max(1, $page - 1);
    $next = min($total_page, $page + 1);

    // First
    $params['page'] = 1;
    $first_url = $_SERVER['PHP_SELF'] . '?' . http_build_query($params);
    ?>

    <li class="page-item <?= $page == 1 ? 'disabled' : '' ?>">
      <a class="page-link" href="<?= htmlspecialchars($first_url) ?>">First</a>
    </li>

    <?php
    // Prev
    $params['page'] = $prev;
    $prev_url = $_SERVER['PHP_SELF'] . '?' . http_build_query($params);
    ?>
    <li class="page-item <?= $page == 1 ? 'disabled' : '' ?>">
      <a class="page-link" href="<?= htmlspecialchars($prev_url) ?>">&laquo; Prev</a>
    </li>
    ...
    // Next
    $params['page'] = $next;
    $next_url = $_SERVER['PHP_SELF'] . '?' . http_build_query($params);
    ?>
    <li class="page-item <?= $page == $total_page ? 'disabled' : '' ?>">
      <a class="page-link" href="<?= htmlspecialchars($next_url) ?>">Next &raquo;</a>
    </li>

    <?php
    // Last
    $params['page'] = $total_page;
    $last_url = $_SERVER['PHP_SELF'] . '?' . http_build_query($params);
    ?>
    <li class="page-item <?= $page == $total_page ? 'disabled' : '' ?>">
      <a class="page-link" href="<?= htmlspecialchars($last_url) ?>">Last</a>
    </li>
